...
def get_node_distance_from_similarity(similarity: float) -> tuple[int, int]:
    ...
    distance = round((max_dist - min_dist) * similarity + min_dist) # TODO: maybe better distance instead of x = y
    return (distance, distance)

# Node as json { keyword: string, position: {x: int, y: int}, data: { sentence: string } }
class Node:
    def __init__(self, keyword: str, position: dict, sentences: list[str], embedding: list):
